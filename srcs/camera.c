...
/*   Updated: 2022/11/22 10:48:25 by jgoad            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cam_generate_rays(t_data *rt)
{
	...
	while (i.y < IMG_H)
	{
		...
		while (i.x < IMG_W)
		{
			ray.x = (rt->cam.view_w * ((float)i.x / (float)IMG_W))
				- (rt->cam.view_w * 0.5f);
			ray.y = (rt->cam.view_h * (1.0f - ((float)i.y / (float)IMG_H)))
				- (rt->cam.view_h * 0.5f);
			ray.z = -1.0f;
			ray.w = 0;
...
void	cam_recalc(t_data *rt)
{
	...
	rt->cam.forward = unit_vec3(vec4_to_vec3(
				mat_mult_vec4(mat_mult_vec4(vec4(0, 0, -1, 0),
						rt->cam.m_pitch), rt->cam.m_yaw)));
