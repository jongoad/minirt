...
While a key is held down, continously move

1. Get keys that are being held down
	- If two opposite keys are held down, discard input for those keys until one is released.
	- When a key is released in this situation, continue applying motion to the key that is still held down.
2. Apply translation to the correct vector based on which key is pressed, using a speed modifier.
	- position += direction_vector * move_speed * time

	Q and E are down and up on the y-axis
	W and S are forward and backwards on the z-axis
	A and D are left and right on the x-axis

Pseudocode:

if (keypress == W_KEY)
	rt->cam.pos += forward_vector * move_speed * time_held
*/
